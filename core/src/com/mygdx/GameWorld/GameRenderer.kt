...
class GameRenderer(val gameWorld: GameWorld) {
    ...
    {
        var newField = Array(9, {i -> Array(11, {hex})})
    ...
    fun render(shouldUpdateGameWorld :Boolean = true) : Int{
        ...
        for (i in 0.. field.width - 1) {
            for (j in 0..field.height - 1) {
                ...
                var curTexture: PolygonRegion = hexArray[i][j]
                if (curHex.activated) {
                ...
                if (curHex.lit) {
                    drawHex(.5f, hexLit, curHex.xl.toFloat(), curHex.yl.toFloat());
                    continue
                }
                if (curHex.occupied && curTexture == hex) {
                    ...
                    drawHex(.35f, curTexture, curHex.xl.toFloat(), curHex.yl.toFloat());
                }
            }
        }
        polygon.end()
    ...
    private fun drawHex(alpha : Float, texture : PolygonRegion, hexX : Float, hexY : Float) {
        val c = polygon.color
        polygon.setColor(c.r, c.g, c.b, alpha)
        polygon.draw(texture, hexX + (-47f + 20 * Math.sqrt(3.0) / 2).toFloat(),
                hexY + (-50f + 20 * Math.sqrt(3.0) / 2).toFloat())
